...
const ManageBlogs = () => {
  ...
  const fetchBlogs = async () => {
    try {
      const response = await axios.get(`https://blogwebsite-3-0tyc.onrender.com/app/manageblogs?page=${currentPage}&limit=3`, {
  ...
  const deleteBlog = async (id) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`https://blogwebsite-3-0tyc.onrender.com/app/deleteBlog/${id}`, { withCredentials: true });
  ...
  const updateBlog = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(`https://blogwebsite-3-0tyc.onrender.com/app/updateBlog/${updatedBlog._id}`, updatedBlog, { withCredentials: true });
